...
// genesEnProceso, idsGenesEnProceso, salidaPredGFF3, salidaEnsEPDGFF3
    ...
    public void inicio(String genesEnProceso, String idsGenesEnProceso, String salidaPredGFF3, String salidaEnsEPDGFF3, String gffVersion, String regionID, String regionInicio, String regionFin, boolean ilpinr, boolean consensos, boolean reporteAbs, int numObjs, int numIter) throws IOException, Exception {
        //--se agarra el archivo de genesEnProceso y se le pasa a la clase utiliti de la libreria-----------       
        ...
        File gtfPredictor = new File(salidaPredGFF3);
        File gffEnsEPD = new File(salidaEnsEPDGFF3);
        gtfPredictor.delete();
        gffEnsEPD.delete();

        // Se imprime encabezado archivo de lecturas en el archivo salida.gtfPredictor
        String versionGFF = "##gff-version\t" + gffVersion + "\n";
        String encabRegion = "##sequence-region\t" + regionID + "\t" + regionInicio + "\t" + regionFin + "\n";

        auxRegistrosEnsembl.guardar(versionGFF + encabRegion, gtfPredictor);
        auxRegistrosEnsembl.guardar(versionGFF + encabRegion, gffEnsEPD);
        ...
        for (int i = 0; i < genIDsize; i++) {
            ...
            this.generaLects("tem", gffEnsEPD, gtfPredictor, ilpinr, consensos, reporteAbs, numObjs, numIter);
            au.delete();
        }
...
//Este constructor recibe la hebra, recibe las lecturas y los boolean de los atg paradas ect , para especificar que es lo que se quiere hacer-----------
    ...
    public void generaLects(String entrada, File gff3EnsemblEPDExt, File gff3Predictor, boolean iLPinr, boolean consensos, boolean reporteAbs, int numObjs, int numIter) throws IOException, Exception {
        ...
        String aux = "gen(" + metaDataGen.get_Secuencia().get(0) + ")."; //se genera gen_prueba
        
        String secuencia = metaDataGen.get_Secuencia().toString();

        int longSec = secuencia.length();

        String secuenciarev = "";

        for (int i = longSec - 1; i >= 0; i--) {
            secuenciarev = secuenciarev + secuencia.charAt(i);
        }
        ...
        analizer.readFromMiddleWare(middle); // Se instancia el objeto constructor de lecturas
        ...
        analizer.constructLectures(
                false); //Se construyen las lecturas (true metodo recursivo, false interactivo).
